...
import { FEED, type Workout } from "../data";
import { PRIMARY_GOAL } from "../goals";
...
const STREAK = {
  ...
  rate: 86,
  bars: [
    ["M", 38, true], ["T", 30, true], ["W", 42, true], ["T", 24, true],
    ["F", 16, false], ["S", 34, true], ["S", 12, false],
...
const FILTER_MAP: Record<string, Workout["type"] | null> = { all: null, gym: "Gym", cardio: "Cardio", sport: "Sport", mobility: "Mobility" };

export function Feed() {
  const s = usePeak();
  const want = FILTER_MAP[s.filter];
  const feed = want ? FEED.filter((f) => f.type === want) : FEED;
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflowY: "auto", padding: "58px 0 104px", animation: "scrIn .28s ease" }}>
      ...
      <div style={{ padding: "6px 22px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <div style={{ fontFamily: mono, fontSize: 11, letterSpacing: "2px", color: "#6b7178", textTransform: "uppercase" }}>Thursday · Jun 14</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#16181d", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 30, padding: "7px 13px 7px 10px" }}>
          ...
          <span style={{ fontFamily: mono, fontWeight: 700, color: "#f4f5f3", fontSize: 15 }}>14</span>
        </div>
      </div>
      ...
      <div style={{ margin: "0 18px 16px", background: "linear-gradient(150deg,#1b2417,#15171d)", border: "1px solid rgba(198,255,61,0.18)", borderRadius: 22, padding: "18px 18px 16px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          <div>
            ...
            <div style={{ fontSize: 22, fontWeight: 700, color: "#f4f5f3", marginTop: 4, letterSpacing: "-0.5px" }}>14-day streak</div>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 7, height: 42, marginTop: 16 }}>
          {STREAK.bars.map((b, i) => (
            <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
              <div style={{ width: "100%", height: b[1], borderRadius: 5, background: b[2] ? "#c6ff3d" : "rgba(255,255,255,0.10)" }} />
              <div style={{ fontFamily: mono, fontSize: 9, color: "#6b7178" }}>{b[0]}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div onClick={() => s.go("goals")} style={{ margin: "0 18px 16px", background: "#16181d", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: 16, cursor: "pointer" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
          ...
          <span style={{ fontSize: 12, color: "#6b7178", fontFamily: mono }}>{PRIMARY_GOAL.progressText}</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <span style={{ fontSize: 20 }}>{PRIMARY_GOAL.icon}</span>
          <div style={{ fontSize: 18, fontWeight: 700, color: "#f4f5f3", letterSpacing: "-0.4px" }}>{PRIMARY_GOAL.name}</div>
        </div>
        <div style={{ margin: "16px 2px 4px" }}>
          <Stepper milestones={PRIMARY_GOAL.milestones} completed={PRIMARY_GOAL.completed} locked={PRIMARY_GOAL.locked} />
        </div>
        <div style={{ fontFamily: mono, fontSize: 11, color: "#9aa0a6", marginTop: 8 }}>Next: {PRIMARY_GOAL.currentMilestone}</div>
      </div>
      ...
      <div style={{ padding: "0 18px", display: "flex", flexDirection: "column", gap: 12 }}>
        {feed.map((w, idx) => {
          ...
          return (
            <div key={idx} style={{ background: "#16181d", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 18, padding: 16, position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 3, background: w.color }} />
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontFamily: mono, fontSize: 10, fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: w.color, background: w.tagBg, padding: "4px 8px", borderRadius: 6 }}>{w.type}</span>
