...
function muscleEstimateToLeafScore(
  ...
  return {
    ...
    percentileRaw: est.percentileRaw,
    cappedPercentile: est.cappedPercentile,
    peakScore: est.peakScore,
    tier: est.tier,
    isPeak: (est.peakScore ?? 0) >= PEAK_CAP,
    buildSnapshot: build,
...
function compositionLeafScore(
  ...
  return {
    ...
    percentileRaw: which.percentileRaw,
    cappedPercentile: which.capped,
    peakScore: peakScoreFromPercentile(which.percentileRaw),
    tier: which.tier,
    isPeak: (peakScoreFromPercentile(which.percentileRaw) ?? 0) >= PEAK_CAP,
    buildSnapshot: build,
    confidence,
    computedAt: asOf,
    state: "measured",
