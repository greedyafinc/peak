import { C, heat, glow, hexA } from "../theme";
  ...
  return (
    <svg viewBox={body.viewBox} style={{ width: "100%", maxWidth: 198, display: "block", margin: "0 auto" }}>
      ...
      {muscles.map((m) => {
        ...
        const filter = isSel
          ? glow.drop(m.untested ? C.blue : baseColor)
          : !m.untested && m.score >= 78
            ? glow.drop(`${baseColor}aa`, 5)
            : "none";
        ...
        if (bands) {
          ...
          return (
            <g key={m.id} style={{ transition: "opacity .2s ease", opacity: dim ? 0.4 : 1, filter }}>
              <clipPath id={clipId}>
                <path d={d} />
              </clipPath>
              <g clipPath={`url(#${clipId})`}>
                ...
                {bands.slice(1).map((b) => (
                  <line key={`dv-${b.id}`} x1={bb.minX - 4} y1={px(b.y0)} x2={bb.maxX + 4} y2={px(b.y0)} stroke={hexA(C.screen, 0.55)} strokeWidth={1.5} />
                ))}
              </g>
              ...
              <path
                ...
                stroke={isSel ? "#ffffff" : hexA(C.screen, 0.5)}
        ...
        return (
          <path
            ...
            strokeWidth={isSel ? 4 : 1.8}
            strokeLinejoin="round"
            style={{ cursor: "pointer", transition: "opacity .2s ease", opacity: dim ? 0.4 : 1, filter }}
          />
        );
      })}
    </svg>
